...
export class AsyncAutocompleteOptions extends Component {
  ...
  getPrompt = () => {
    ...
    if (loading) {
      return (
        <AutocompletePrompt>
          <FormattedMessage id={'InputTagsSearch.loading'} defaultMessage={'Loading ...'} />
        </AutocompletePrompt>
      );
    }
    ...
    if (!options.length && !this.props.creatable) {
      return (
        <AutocompletePrompt>
          <FormattedMessage id={'InputTagsSearch.notFound'} defaultMessage={'Nothing found'} />
        </AutocompletePrompt>
      );
    }
  ...
  renderItems = () => {
    ...
    const { inputValue, renderItem } = this.props;
